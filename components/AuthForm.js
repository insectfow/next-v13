...
const AuthForm = () => {
  ...
  const createUser = () => {
    setPersistence(authService, autoLogin ? browserLocalPersistence : browserSessionPersistence)
      .then(async () => {
        try {
      ...
      <form className="auth-form" onSubmit={onSubmit}>
        ...
        <div className="auth-input-box pw">
          ...
          <button type="button" className="eyes-box" onClick={onToggleShowPw}>
            <Image width={36} height={36} src={isEyes ? eyesOn : eyesOff} alt="eyes icon" />
          </button>
        </div>
        <label>
        ...
        {error?.code && <p className="error">{error.code + ' : ' + error.message}</p>}
      </form>
